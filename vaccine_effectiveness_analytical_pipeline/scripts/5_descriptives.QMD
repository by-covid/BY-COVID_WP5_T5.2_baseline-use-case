...
```{r, unadjusted survival, results='hide', message=FALSE, warning=FALSE, error=FALSE}
...
# df_notmatched <- df_notmatched %>% mutate(status=ifelse(status==1,TRUE,FALSE)) %>% filter(futime>=0)
...
```{r, survival plot cumulative events table, message=FALSE, warning=FALSE, error=FALSE}

res <- summary(fit_unadj_surfit, times = c(0,100*(1:6)))
