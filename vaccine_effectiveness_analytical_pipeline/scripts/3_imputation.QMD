...
if(FALSE) {
  ...
  input      <- list('cohort_data and missing_profile (database tables in BY-COVID-WP5-BaselineUseCase-VE.duckdb)')
  output     <- list('3_imputation.html (report)','cohort_data_imputed (database table in BY-COVID-WP5-BaselineUseCase-VE.duckdb)','flag_listwise_del (variable in cohort_data table)')
}
...
tryCatch(
  {
    ...
    df_core <- dbGetQuery(con,
      "SELECT person_id, age_nm,sex_cd,residence_area_cd,pregnancy_bl,essential_worker_bl,institutionalized_bl,foreign_bl FROM cohort_data")
    dt_prof <- dbGetQuery(con, "SELECT * FROM missing_profile")
  },
  finally={
...
v_equal <- c()
for(i in colnames(df_core)) {
  if(length(unique(df_core[[i]]))==1) {
    v_equal <- c(v_equal,i)
  }
}
ifelse(df_var_imputation_method[which(df_var_imputation_method$Variable_name=="pregnancy_bl"),"All_missing_values"],
       mcar_res <- naniar::mcar_test(df_core %>% select(any_of(setdiff(colnames(df_core),v_equal))) %>% select(-c(person_id,pregnancy_bl))),
       mcar_res <- naniar::mcar_test(df_core %>% select(any_of(setdiff(colnames(df_core),v_equal))) %>% select(-person_id)))
...
df_var_imputation_method <- df_var_imputation_method %>%
  mutate(imputation_method = case_when(
    ...
    Missing_values==TRUE & Required==TRUE & Comorbidity==FALSE & Core==TRUE & All_missing_values==FALSE & cond_mcar==FALSE & Perc_miss_lt==TRUE ~ "Imputation of missing values (not MCAR)",
    Missing_values==TRUE & Required==TRUE & Comorbidity==FALSE & Core==TRUE & All_missing_values==FALSE & cond_mcar==FALSE & Perc_miss_lt==FALSE & Perc_miss_lt15==TRUE ~ "Imputation of missing values (not MCAR, need for sensitivity analysis)",
...
if(length(v_ld)>0) {
  tryCatch(
    {
      tmp_ld_rn <- data.frame(person_id=df_core[which(is.na(df_core[,v_ld])),"person_id"])
      con = dbConnect(duckdb::duckdb(), dbdir=auxilary_database_path, read_only=FALSE)
...
tmp <- method_imputation(df_core,v_imp)
data_imp <- tmp[[1]]
...
tryCatch(
  {
    con = dbConnect(duckdb::duckdb(), dbdir=auxilary_database_path, read_only=FALSE)
    count_ld_true <- dbGetQuery(conn = con, "SELECT COUNT(*) FROM cohort_data WHERE flag_listwise_del==TRUE")
    count_ld_false <- dbGetQuery(conn = con, "SELECT COUNT(*) FROM cohort_data WHERE flag_listwise_del==FALSE")
