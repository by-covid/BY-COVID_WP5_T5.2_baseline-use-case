...
if(FALSE) {
  ...
  output     <- list('1_DQA.html (report)','missing_profile (database table in BY-COVID-WP5-BaselineUseCase-VE.duckdb)')
}
...
tryCatch(
  {
    con = dbConnect(duckdb::duckdb(), dbdir=auxilary_database_path, read_only=FALSE)
    ## create 'missing_profile' table
    dbExecute(con, "CREATE OR REPLACE TABLE missing_profile (
                    feature VARCHAR,  
                    num_missing INT,
                    pct_missing DOUBLE)")
    dbWriteTable(con, "missing_profile",dt_prof,overwrite = TRUE, append=FALSE)
    # print(dbGetQuery(conn = con, "SELECT * FROM missing_profile"))
  },
  finally={
    dbDisconnect(con, shutdown=TRUE)
  })
